...
def sample_dataset_requests(
                ...
                if len(prompt_token_ids) + len(completion_token_ids) < 12800:
                    ...
                    for i in range(0, len(data["conversations"]) - 1, 2):
                        if i + 1 < len(data["conversations"]):
                            human_prompt = data["conversations"][i]["value"]
                            ai_response = data["conversations"][i + 1]["value"]
                            all_human_prompts.append(human_prompt)
                            all_ai_responses.append(ai_response)
                    
                    if all_human_prompts and all_ai_responses:
                        # Concatenate all human prompts with newlines
                        concatenated_human_prompt = "\n\n".join(all_human_prompts)
                        # Concatenate all AI responses with newlines  
                        concatenated_ai_response = "\n\n".join(all_ai_responses)
                        
                        prompt_token_ids = tokenizer(concatenated_human_prompt).input_ids
                        completion_token_ids = tokenizer(concatenated_ai_response).input_ids
                        
                        # Truncate very long concatenated prompts to reduce attention instability
                        prompt_cap = 6000
                        if len(prompt_token_ids) > prompt_cap:
                            prompt_token_ids = prompt_token_ids[:prompt_cap]
                            concatenated_human_prompt = tokenizer.decode(prompt_token_ids)
                            # Re-tokenize to ensure consistency
                            prompt_token_ids = tokenizer(concatenated_human_prompt).input_ids
                        
                        # Cap the expected response length to keep server satisfied
                        cap_len = 500 #TODO: make this configurable
                        if len(completion_token_ids) > cap_len:
                            completion_token_ids = completion_token_ids[:cap_len]
                        
                        # Only accept if total tokens remain within reasonable bound
                        if len(prompt_token_ids) + len(completion_token_ids) < 12800: #TODO: make long prompts break into multiple requests
                            prompts.append(concatenated_human_prompt)
                            completions.append(concatenated_ai_response)
                            prompt_lens.append(len(prompt_token_ids))
                            response_lens.append(len(completion_token_ids))
                            total_prompts_extracted += 1
    ...
    # Truncate to exact number of requests if we have more
    if len(prompts) > num_requests:
        prompts = prompts[:num_requests]
        completions = completions[:num_requests]
        prompt_lens = prompt_lens[:num_requests]
        response_lens = response_lens[:num_requests]

    print(f"  Processed {conversation_count} conversations, extracted {total_prompts_extracted} total prompts")
...
def sample_dataset_conversations_all_wait(
    dataset_path: str,
    max_conversations: int,
    tokenizer,
    per_turn_resp_cap: int = 512,
    total_token_cap: int = 12800,
):
    # Sanitize caps
    try:
        per_turn_resp_cap = int(per_turn_resp_cap) if per_turn_resp_cap is not None else 512
    except Exception:
        per_turn_resp_cap = 512
    if per_turn_resp_cap <= 0:
        per_turn_resp_cap = 512
    try:
        total_token_cap = int(total_token_cap) if total_token_cap is not None else 12800
    except Exception:
        total_token_cap = 12800
    if total_token_cap <= 0:
        total_token_cap = 12800
    # Returns a list of conversations, each as a list of (prompt_text, prompt_len, response_len)
    conversations = []
    with open(dataset_path) as f:
        for line in f:
            ...
            for i in range(0, len(conv) - 1, 2):
                ...
                human = conv[i]["value"]
                ai = conv[i + 1]["value"]
                prompt_ids = tokenizer(human).input_ids
                resp_ids = tokenizer(ai).input_ids
                # Cap response per turn
                if len(resp_ids) > per_turn_resp_cap:
                    resp_ids = resp_ids[:per_turn_resp_cap]
                # Require reasonable total per turn to avoid server failures
                if len(prompt_ids) + len(resp_ids) < total_token_cap:
                    turns.append((human, len(prompt_ids), len(resp_ids)))
            if turns:
                conversations.append(turns)
    return conversations

async def benchmark_all_wait(
    tokenizer,
    ...
    prefill_SLO: float,
    priority_ratio: float,
    alpha_tokens_per_sec: float = 4.0,
):
    # Reuse existing measurement wrappers
    m = MeasureLatency()
    m1 = MeasureLatency()
    query_model = m.measure(query_model_vllm)
    query_model1 = m1.measure(query_model_vllm)

    conversation_latencies: List[float] = []

    # Generator yielding conversations respecting distribution
    async def async_conversation_gen(conversations_iter, qps: float, distribution: str, coefficient_variation: float):
        for conv in conversations_iter:
            yield conv
            if distribution != "burst":
                await asyncio.sleep(get_wait_time(1.0 / max(qps, 1e-6), distribution, coefficient_variation))

    # For priority support compatibility (keep single priority type=0)
    async def conversation_runner(conv_turns, session_id):
        tasks_local = []
        conv_start = time.time()
        for (prompt_text, prompt_len, resp_len) in conv_turns:
            # expected response len per turn
            ...
            try:
                observed_resp_len = output.get('response_len', resp_len)
            except Exception:
                observed_resp_len = resp_len
            wait_seconds = float(observed_resp_len) / max(alpha_tokens_per_sec, 1e-6)
            await asyncio.sleep(wait_seconds)
        conv_dur = time.time() - conv_start
        conversation_latencies.append(conv_dur)
        return tasks_local

    start_time = time.time()
    # Launch conversations according to traffic pattern
    tasks = []
    async for conv in async_conversation_gen(iter(conversations), qps=qps, distribution=distribution, coefficient_variation=coefficient_variation):
        import uuid
        session_id = str(uuid.uuid4().hex)
        tasks.append(asyncio.create_task(conversation_runner(conv, session_id)))

    # Gather all tasks; flatten results to match existing metric flow
    conv_results = await asyncio.gather(*tasks)
    queries = []
    for tr_list in conv_results:
        # tr_list is a list of (prompt, output) tuples returned above
        for tr in tr_list:
            # The conversation_runner already awaited and received (prompt, output)
    ...
    median_token_latency = np.median(m._per_token_latencies) if m._per_token_latencies else 0.0
    median_e2e_latency = np.median(m._latencies) if m._latencies else 0.0
    median_inference_latency = np.median(m._inference_latencies) if m._inference_latencies else 0.0

    throughput = calculate_throughput(queries, dur_s, GenerationBackend.vLLM, tokenizer,
                                      median_token_latency, median_e2e_latency, median_inference_latency,
                                      m._latencies, m._per_token_latencies, m._inference_latencies, m._request_ids, m._decode_latencies, m._request_lens,
                                      True, False)
    calculate_cdf(m._latencies)
...
def main():
    ...
    parser.add_argument('--conversation_mode', choices=["first", "all", "all-wait"], default="first",
                        help="How to sample conversations from the dataset. 'first' uses only the first human prompt and AI response. 'all' concatenates all human prompts and all AI responses into single, longer prompt-response pairs. 'all-wait' uses all human prompts and waits for the AI response.")
    ...
    if args.conversation_mode == "all-wait":
        # For all-wait mode, we need to sample conversations and then run them sequentially
        # The sample_dataset_conversations_all_wait function already handles sampling and truncating
        # We just need to pass the sampled conversations to the benchmark_all_wait function
        # Derive a robust per-turn cap from fixed_max_tokens or default to 512
        per_turn_cap = 512
        try:
            if args.fixed_max_tokens is not None and int(args.fixed_max_tokens) > 0:
                per_turn_cap = int(args.fixed_max_tokens)
        except Exception:
            per_turn_cap = 512
        sampled_conversations = sample_dataset_conversations_all_wait(
            args.prompts_filename,
            args.random_prompt_count,
            tokenizer,
            per_turn_resp_cap=per_turn_cap,
            total_token_cap=12800,
        )
        (throughput, prefill_token_latencies, decode_token_latencies, inference_latencies, avg_instance_num, request_latencies, request_ids, decode_latencies, request_lens,
         conversation_latencies) = asyncio.run(benchmark_all_wait(
            tokenizer,
